...
class WGAN():
    ...
    def generator(self, input_):
        ...
        # return tanh instead of sigmoid
        return tf.nn.tanh(output)

    def discriminator(self, input_, target, stride = 2, layer_count = 4):
        """
        Using the Patchwgan as a discriminator
        """
    ...
    def build_model(self):
        ...
        with tf.name_scope('DiscriminatorLoss'):
            #since it has to minimise it, we take negative
            self.D_loss = -(tf.reduce_mean(self.predict_real + self.EPS) - tf.reduce_mean(self.predict_fake + self.EPS))
        ...
        with tf.name_scope('GeneratorLoss'):
            # min -E[critic(fake)]
            self.wgan_loss = -tf.reduce_mean(self.predict_fake + self.EPS)
            ...
            self.G_loss = self.wgan_wt * self.wgan_loss + self.l1_wt * self.l1_loss + self.vgg_wt * self.vgg_loss

        with tf.name_scope('Summary'):
            D_loss_sum = tf.summary.scalar('Discriminator Loss', self.D_loss)
            G_loss_sum = tf.summary.scalar('Generator Loss', self.G_loss)
            wgan_loss_sum = tf.summary.scalar('wgan Loss', self.wgan_loss)
            l1_loss_sum = tf.summary.scalar('L1 Loss', self.l1_loss)
            vgg_loss_sum = tf.summary.scalar('VGG Loss', self.wgan_loss)
            ...
            self.G_summary = tf.summary.merge([wgan_loss_sum, l1_loss_sum, vgg_loss_sum, G_loss_sum])
    ...
    def train(self):
        ...
        with tf.Session() as self.sess:
            ...
            loss_operations = [self.D_loss, self.G_loss, self.wgan_loss, self.l1_loss, self.vgg_loss]
            ...
            for i in range(start, self.epochs):
                ...
                for j in range(batches):
                    ...
                    if j % logger_frequency == 0:
                        D_loss, G_loss, wgan_loss, L1_loss, VGG_loss = self.sess.run(loss_operations, feed_dict=feed_dict)

                        wgan_loss = wgan_loss * self.wgan_wt
                        ...
                        line = 'Batch: %d, D_Loss: %.3f, G_Loss: %.3f, wgan: %.3f, L1: %.3f, P: %.3f' % (
                            j, D_loss, G_loss, wgan_loss, L1_loss, VGG_loss)
                        print(line)
    ...
    def validate(self):
        ...
        for i in range(val_image_count):
            ...
            psnr = peak_signal_noise_ratio(real_B, generated_B)
            ssim = structural_similarity(real_B, generated_B, multichannel = True)
        ...
        if(score > self.score_best):
            ...
            self.saver.save(self.sess, os.path.join(self.ckpt_dir, 'wgan'), global_step = self.step.eval())
